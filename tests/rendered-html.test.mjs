...
test("server-renders the crypto leaderboard", async () => {
  ...
  assert.doesNotMatch(html, /\/api\/project-icon/);
  assert.match(html, /href="\/api\/click\/hyperliquid"/);
  assert.match(html, /href="\/submit\?boost=hyperliquid&amp;target=/);
